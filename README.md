...
`machine_c_convert_loop.sh` runs split-specific processing by default: clean-only event simulation on `train` (which includes val subset sequences), and full corruption/noise + event simulation on `test`.
